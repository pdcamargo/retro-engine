...
import { UI_GLYPH_BYTE_SIZE, UI_GLYPH_FLOAT_COUNT } from './ui-glyph-instance';
import { UI_TEXT_WGSL } from './ui-text.wgsl';

const QUAD_VERTICES = new Float32Array([0, 0, 1, 0, 1, 1, 0, 1]);
const QUAD_INDICES = new Uint16Array([0, 1, 2, 0, 2, 3]);

/** One contiguous run of glyph instances sharing a font atlas. */
export interface UiGlyphBatch {
...
/**
 * Render-world resource owning the in-game UI text pipeline: the shared unit
 * quad, a growable per-glyph instance buffer + scratch, the format-specialized
 * MSDF pipeline, and a per-atlas bind-group cache. Built lazily on the first
 * frame with a surface.
 */
export class UiTextPipeline {
  ...
  /** Glyph instances packed for the current frame. */
  count = 0;
  /** Per-atlas draw batches for the current frame. */
  readonly batches: UiGlyphBatch[] = [];

  private readonly bindGroupCache = new Map<number, CachedAtlasBindGroup>();
  ...
  ensureInitialised(renderer: Renderer, format: TextureFormat): boolean {
    ...
    if (this.atlasBindGroupLayout === undefined) {
      this.atlasBindGroupLayout = renderer.createBindGroupLayout({
        label: 'ui-text-atlas-layout',
        entries: [
    ...
    if (this.shaderModule === undefined) {
      this.shaderModule = renderer.createShaderModule({ label: 'retro_ui::text', code: UI_TEXT_WGSL });
    }
    if (this.pipelineLayout === undefined) {
      this.pipelineLayout = renderer.createPipelineLayout({
        label: 'ui-text-layout',
        bindGroupLayouts: [this.atlasBindGroupLayout],
      });
    }
    ...
    this.pipeline = renderer.createRenderPipeline({
      ...
      vertex: {
        ...
        buffers: [
          ...
          {
            arrayStride: UI_GLYPH_BYTE_SIZE,
            stepMode: 'instance',
            attributes: [
              { shaderLocation: 1, format: 'float32x4', offset: 0 },
              { shaderLocation: 2, format: 'float32x4', offset: 16 },
              { shaderLocation: 3, format: 'float32x2', offset: 32 },
              { shaderLocation: 4, format: 'unorm8x4', offset: 40 },
            ],
          },
        ],
      },
  ...
  ensureCapacity(renderer: Renderer, glyphCount: number): void {
    if (glyphCount <= this.instanceCapacity && this.instanceBuffer !== undefined) return;
    let capacity = this.instanceCapacity > 0 ? this.instanceCapacity : 256;
    while (capacity < glyphCount) capacity *= 2;
    this.instanceBuffer?.destroy();
    this.instanceBuffer = renderer.createBuffer({
      label: 'ui-text-instance',
      size: capacity * UI_GLYPH_BYTE_SIZE,
      usage: BufferUsage.VERTEX | BufferUsage.COPY_DST,
    });
    const buffer = new ArrayBuffer(capacity * UI_GLYPH_FLOAT_COUNT * 4);
    this.scratchF32 = new Float32Array(buffer);
    this.scratchU32 = new Uint32Array(buffer);
    this.instanceCapacity = capacity;
  }

  /** Look up (or build + cache) the atlas bind group for a font's atlas image. */
  bindGroupFor(atlas: Handle<Image>, renderImages: RenderImages, renderer: Renderer): BindGroup | undefined {
    ...
    const bindGroup = renderer.createBindGroup({
      label: `ui-text-atlas#${atlas.index}`,
    ...
    this.bindGroupCache.set(atlas.index, { bindGroup, source: current });
    return bindGroup;
  }
