...
class FakeMaterial {
  ...
  static readonly bindGroup = MaterialSchema(FakeMaterial, [
    ...
    { kind: 'texture', binding: 1, visibility: 'fragment', fieldKey: 'baseColorTexture' },
  ]);
}
...
describe('prepareBindGroup', () => {
  it('packs uniforms and binds the texture', () => {
    const renderer = makeRecordingRenderer();
    const layout = schemaToBindGroupLayout(renderer, FakeMaterial.bindGroup, 'fake');
    const stubView: TextureView = { destroy: () => undefined };
    const material = new FakeMaterial();
    material.baseColorTexture = stubView;
    const scratch = new ArrayBuffer(64);
    const prepared = prepareBindGroup(renderer, FakeMaterial.bindGroup, layout, material, undefined, scratch, 'fake');
    ...
    expect(bgDesc.entries[1]?.resource).toBe(stubView);
  });

  it('throws when a referenced texture field is undefined', () => {
    const renderer = makeRecordingRenderer();
    const layout = schemaToBindGroupLayout(renderer, FakeMaterial.bindGroup, 'fake');
    const material = new FakeMaterial(); // baseColorTexture is undefined
    const scratch = new ArrayBuffer(64);
    expect(() =>
      prepareBindGroup(renderer, FakeMaterial.bindGroup, layout, material, undefined, scratch, 'fake'),
  ...
  it('reuses the uniform buffer across prepares of the same material', () => {
    ...
    material.baseColorTexture = stubView;
    const scratch = new ArrayBuffer(64);
    const first = prepareBindGroup(renderer, FakeMaterial.bindGroup, layout, material, undefined, scratch);
    material.metallic = 0.9;
    const second = prepareBindGroup(renderer, FakeMaterial.bindGroup, layout, material, first, scratch);
