 ...
 * Material2d instances of type `M`. Inserted as a main-world resource by
 * `Material2dPlugin<M>`.
 *
 * Type alias of {@link Materials} — the store shape is identical to the 3D
 * store. The 2D / 3D distinction is at the {@link Material2d} trait level
 ...
 * from any other store instance. The runtime class identity is the subclass;
 * this alias just narrows the generic parameter.
 *
 * @example
 * ```ts
