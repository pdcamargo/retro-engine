...
- `ThemeTokens` / `defaultTokens` / `applyTheme` — token-driven theming. The Retro Engine design-system theme has landed: `ThemeTokens = { palette, metrics }`, where `RetroPalette` (~21 sRGB colors, phosphor green on cool charcoal) is the canonical reskin knob and `applyTheme` maps it onto the full `ImGuiCol_` slot map + style vars.
- **Fonts** — `SurfaceOverlay.loadFont`, `registerFonts` / `FontSpec`, a `fonts` plugin option, and `ui.withFont(name, size, body)`. Validated in the playground: JetBrains Mono is the default editor face (size-scalable, 13px base) and Silkscreen is used as a pixel display face for headings. Font bytes are consumer-supplied (the playground fetches them from a dev-server `/fonts` route); editor-sdk bundles none.
