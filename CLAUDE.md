...
## 14. Editor tooling (MCP)

The studio exposes an MCP surface so AI agents drive the editor through editor-shaped tools instead of blind file edits. Architecture and decisions are authoritative in [ADR-0109](docs/adr/ADR-0109-studio-mcp-transport-and-architecture.md): an AI client launches the `@retro-engine/studio-mcp-server` relay; the studio connects to it as a reconnecting WebSocket client; commands are defined once in `@retro-engine/editor-mcp` and the relay advertises them as MCP tools automatically.

- **Prefer the MCP for editor-shaped work when it's connected.** Reading/editing the live scene, entities, components, selection, and history is what the tools are for — reach for `entity.spawn` / `component.set` / `hierarchy.reparent` / `scene.save` over hand-editing `.rescene` files while the studio owns the world. Verify changes with `selection.inspected` / `entity.get` / `studio.state` (and screenshots, when added) rather than Playwright pixel probes.
- **Keep the registry fed.** Every new editor-facing capability — and every authored engine component that gains editor actions (cf. §13) — gets a matching `defineCommand(...)` in `packages/editor-mcp/src/commands/`. A capability with no command is a tracked gap, not a default. Adding the command is the whole change; the tool list follows.
- **Writes are no-friction but accountable.** Mutating commands run immediately, route through the editor `History` (so `history.undo` reverts them), and are recorded in the audit ring (MCP panel + `studio.audit`). Batch related destructive edits so they undo as one step where it makes sense.
- **`studio.eval` is gated and never load-bearing.** It runs only when the user enables "Allow eval" in the MCP panel (`mcp.eval`), and exists for ad-hoc inspection/poking — never rely on it in shipped code paths or as a substitute for a typed command.
- **The bridge ships opt-in, not stripped.** Default on in dev, off in prod, toggled in the MCP panel — do not reintroduce a build-time strip. Command handlers run inside the studio (browser/webview), so `packages/editor-mcp` stays browser-safe; the relay (`packages/studio-mcp-server`) is node/bun-only and depends solely on `@retro-engine/mcp-protocol` for wire types.
